...
{
    ShowError(GetMessage("Не загружен модуль инфоблоков"));
    return;
}
...
$doctors = [["name" => "Ввести идентификатор вручную", "value" => ""]];

$arSelect = Array("NAME","PROPERTY_*", "IBLOCK_ID", "ID");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
...
while($ob = $res->GetNextElement()){ 
    ...
    $doctors[] = ["name" => $arFields["NAME"], "value" => $arProps["MEDIC_ID"]["VALUE"]];
}
...
return array(
    ...
    'nodes' => array(
        '.UMC-widget__medic-id' => array(
            'name' => "Ручной ввод идентификатора",
            'type' => 'text',
        ),
        '.UMC-widget__private-link' => array(
			'name' => 'Ссылка на пользовательское соглашение',
			'type' => 'link',
		),
    ),
    'style' => array(
